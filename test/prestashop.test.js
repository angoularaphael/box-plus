...
test('etats-unis — fiche neuve à Minimes, existante cherchée sur États-Unis', () => {
  ...
  assert.equal(gymConfigFromZoneId('3').deciplus_label, 'Portet');
  assert.equal(gymConfigFromZoneId('4').deciplus_label, 'Ramonville');
  assert.equal(gymConfigFromZoneId('5').deciplus_label, 'St-Cyprien');
  ...
  assert.match(indexSrc, /Migration Balma → Minimes obligatoire avant vente/);
  assert.match(indexSrc, /let gymConfig = getGymConfig\(order\.gym\)/);
  assert.match(indexSrc, /order\.force_new_member = order\.force_new_member !== false/);

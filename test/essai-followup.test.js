...
test('relance client J+0 / J+1 / J+2 puis stop à J+3', () => {
  ...
  assert.match(copy.subject, /^Camille, il reste quelques places/);
  assert.match(copy.html, /Bonjour Camille/);
  assert.doesNotMatch(copy.html, /🚨|DERNIÈRES PLACES|Voir les offres|emails promotionnels/);
...
test('dispatch : relance client avant J+3, 1 WhatsApp max', async () => {
  ...
  assert.match(mails[0].subject, /^Camille, il reste quelques places/);

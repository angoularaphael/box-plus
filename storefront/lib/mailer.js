  ...
  <table style="width:100%;border-collapse:collapse;margin:20px 0">
    ...
    ${
      order.addons?.blade?.status === 'paid'
        ? `<tr><td style="padding:8px;border-bottom:1px solid #eee"><strong>Gants Blade ${escapeHtml(order.addons.blade.color_label || '')} ${escapeHtml(order.addons.blade.size || '')}</strong></td><td style="padding:8px;border-bottom:1px solid #eee">17,90 € — possibilité de retrait dès le jour même à ${escapeHtml(/cyprien/i.test(order.addons.blade.pickup_gym || '') ? 'Saint-Cyprien' : 'Minimes')} (12h–14h et 17h–21h, sam. 15h–18h)</td></tr>`
        : ''
    }
  </table>
  <p><strong>Pour bien démarrer :</strong></p>

...
function offerDocumentCopy(product = {}, order = {}) {
  ...
  if (kind === 'essai') {
    ...
    details = [
      `Abonnement cours collectifs + accès libre aux 5 salles Boxing Center${period ? ` — ${period}` : ''}.`,
      'Paiement en une fois — pas de prélèvement automatique.',
    ];
    accessLine = 'Accès : cours collectifs + accès libre 6j/7 (10h–21h) sur les 5 salles Boxing Center.';
    ...
    accessLine = 'Accès : cours collectifs + accès libre 6j/7 (10h–21h) sur les 5 salles Boxing Center.';
    showSepa = true;
    showBadge72h = productNeedsAutoBadge(product);
  } else {
    headline = title || 'Abonnement Boxing Center';
    details = [
      `Abonnement cours collectifs + accès libre aux 5 salles Boxing Center${period ? ` — ${period}` : ''}.`,
    ];
    accessLine = 'Accès : cours collectifs + accès libre 6j/7 (10h–21h) sur les 5 salles Boxing Center.';

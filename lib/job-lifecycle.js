...
/**
 * Séance offerte = fiche membre sans vente Deciplus.
 * Un job success sans sale_id n’est pas une revue manuelle.
 */
function lifecycleFromBotOutcome(outcome = {}) {
  const status = String(outcome.status || '').toLowerCase();
  if (status === 'success' || status === 'completed') return STATES.VERIFIED;
  if (outcome.deciplus_sale_id) return STATES.VERIFIED;
  return STATES.MANUAL_REVIEW;
}

...
function buildProductConfig(order, matchedProduct = null) {
  ...
  const deciplusLabel = payplug4xPrelev
    ? PAYPLUG_4X_DECIPLUS_LABEL
    : prestation
      ? prestation.label
  ...
  return {
    ...
    amount: order.payment?.amount || prestation?.amount || matchedProduct.price,
    ...typeDefaults,
    sale_type: saleType,
...
function scoreCatalogTile(text, productConfig = {}) {
  ...
  if (productConfig.payplug_4x_prelevement === true) {
    if (/259.*4\s*[x×].*prelevement|4\s*[x×].*prelevement.*259/i.test(normalized)) score += 320;
    else if (/prelevement|pr[eé]l[eè]vement/.test(normalized) && /259/.test(raw)) score += 200;
    if (/OFFRE PROMO 12MOIS 259/i.test(raw) && !/4\s*[x×]|prelevement|pr[eé]l[eè]vement/i.test(raw)) {
      score -= 120;
    }
  }

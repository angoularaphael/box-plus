...
function buildCarteFallbackConfig(order, defaults) {
  ...
  return {
    ...
    product_id: productId || (isEssai ? 'seance-essai' : null),
...
function buildProductConfig(order, matchedProduct = null) {
  ...
  if (isCarteMerchOrder(order)) {
    saleType = 'carte';
    if (/\bbadge\b/i.test(String(matchedProduct.title || '')) && !/essai|coaching/i.test(String(matchedProduct.title || ''))) {
      return buildCarteFallbackConfig(order, defaults);
    }
  }
  ...
  return {
    ...
    product_id: String(order.product_id || matchedProduct.id),

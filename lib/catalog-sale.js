...
function buildProductConfig(order, matchedProduct = null) {
  ...
  if (!matchedProduct) {
    const name = normalizeText(order.product_name || order.offer || order.product_id);
    const paidEssai =
      Number(order.payment?.amount) > 0 &&
      (name.includes('essai') || String(order.product_id || '').includes('seance-essai'));
    if (paidEssai || order.sale_type === 'carte') {
      const typeDefaults = defaults.carte || defaults.abonnement;
      return {
        key: 'essai',
        label: order.product_name || "SEANCE D'ESSAI",
        deciplus_product_name: order.product_name || "SEANCE D'ESSAI",
        deciplus_product_search: order.deciplus_product_search || 'essai',
        amount: order.payment?.amount || 10,
        ...typeDefaults,
        sale_type: 'carte',

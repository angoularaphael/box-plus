...
function buildProductConfig(order, matchedProduct = null) {
  ...
  const comptantStyleProduct = isComptantStyleProduct({
    id: order.product_id || order.product_reference,
    name: matchedProduct.title,
    display_name: order.product_name,
    supports_installment_choice: order.product_snapshot?.supports_installment_choice,
    badge: order.product_snapshot?.badge,
  });
  const explicitRibPrelev =
    payplug4xPrelev || billingPlan === 'rib' || (paymentPlan === '4x' && billingPlan === 'rib');
  ...
  const comptant =
    !explicitRibPrelev &&
    (forceCarteComptant ||
    (comptantStyleProduct && !explicitRibPrelev) ||
  ...
  return {
    ...
    create_sale: true,
    auto_badge: payplug4xPrelev ? false : saleType === 'abonnement' && !comptant,
  };
}

...
function offerToProductId(offer) {
  ...
  if (raw === '29' || raw === 'offre_29' || raw === 'duo') return 'offre-duo';
...
function validateBalmaSwitchPayload(body = {}) {
  ...
  const product = none ? null : findBalmaPrelevementOffer(offer);
  const offerMissing = 'Choisis l’offre en prélèvement que tu avais à Balma, ou indique que tu n’as pas d’offre active';
...
function buildBalmaSwitchOrder({ first_name, last_name, birthdate, offer, skip_restore }) {
  const stamp = Date.now();
  const none = isNoneOffer(offer) || skip_restore;
  return {

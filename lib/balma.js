...
function validateBalmaSwitchPayload(body = {}) {
  ...
  const product = findBalmaPrelevementOffer(offer);
  ...
  if (!product) {
    errors.push('Choisis une offre en prélèvement');
  }
  return {
    errors,
    first_name,
    ...
    birthdate,
    prelevement,
    offer: product ? product.legacy_id || product.product_id || product.id || offer : offer,
    product,
  };
}

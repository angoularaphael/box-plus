...
/** Scalapay : encaissement total, vente Deciplus en comptant (pas la tuile 4× prélèvement). */
function isScalapayOrder(order = {}) {
  const payment = order.payment || {};
  const plan = String(order.payment_plan || payment.payment_plan || '').toLowerCase();
  if (plan === 'scalapay') return true;
  const method = String(payment.method || order.payment_method || '').toLowerCase();
  return method === 'scalapay';
}

/** 3 ou 4 selon le choix client sur Scalapay (PayPlug / commande). */
function scalapayInstallmentCount(source = {}) {
  const payment = source.payment || source;
  const pm = payment.payment_method || source.payment_method || {};
  const candidates = [
    payment.scalapay_installments,
    source.scalapay_installments,
    payment.installment_count,
    pm.installment_count,
    pm.installments,
    pm.nb_installments,
    Array.isArray(pm.schedule) ? pm.schedule.length : null,
    Array.isArray(payment.installments) ? payment.installments.length : null,
  ];
...
function scalapayInfoComptaNote(order = {}) {
  const n = scalapayInstallmentCount(order);
  if (n === 3) return '3× Scalapay';
  if (n === 4) return '4× Scalapay';
  return '3×/4× Scalapay';
}

/** Tuile Deciplus comptant pour un paiement Scalapay (259 € = OFFRE PROMO 12 MOIS). */
function resolveScalapayDeciplus(product = {}, order = {}) {
  const { id, legacy, name, euros } = productKeyHints(product, order);
  const paid = Number(order.payment?.amount || order.amount || 0);
  const amountHint = euros || (Number.isFinite(paid) && paid > 0 ? Math.round(paid) : null);

  if (
    id === 'offre-saison' ||
    legacy === 'offre-saison' ||
    id === 'dp-100' ||
    /OFFRE\s*PROMO\s*12\s*MOIS/.test(name) ||
    amountHint === 259
  ) {
    return {
  ...
  if (
    ...
    return {
      deciplus_product_name: 'BABY BOXE',
      deciplus_product_search: 'BABY BOXE',
      amount: 250,
    };
  }
  ...
  if (
    ...
    return {
      deciplus_product_name: 'BOXE EDUCATIVE',
      deciplus_product_search: 'BOXE EDUCATIVE',
      amount: 295,
    };
  }
  ...
  if (
    ...
    /COMPTANT\s*12\s*MOIS/.test(name) ||
    amountHint === 400
  ) {
    return {
      deciplus_product_name: 'COMPTANT 12 MOIS',
      deciplus_product_search: 'COMPTANT 12',
      amount: 400,
    };
  }
...
/** Boxe éducative / compétition — à Portet, encaissement Noble Art Portésien (PayPal), pas Végé/CAWL. */
function isNoblartPaypalOfferProduct(product = {}) {
  if (!product) return false;
  ...
  if (/comp[eé]tit/i.test(`${id} ${legacy} ${title}`)) return true;
  return false;
}
...
/** Offres éligibles au paiement fractionné Scalapay (PayPlug). */
function productSupportsScalapay(product = {}) {
  if (!product) return false;
...
/** Scalapay exclu pour Portet (PayPal 4× sans frais à la place). */
function gymSupportsScalapay(gym) {
  const g = String(gym || '')
    .trim()
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');
  if (!g) return true;
  return g !== 'portet' && !g.includes('portet');
}

function supportsScalapayCheckout(product, gym) {
  return productSupportsScalapay(product) && gymSupportsScalapay(gym);
}

function normalizePaymentPlan(raw, product) {
  const plan = String(raw || '').trim().toLowerCase();
  if (plan === 'scalapay' && productSupportsScalapay(product)) return 'scalapay';
  if (!productSupportsInstallmentChoice(product) && !productSupportsScalapay(product)) return null;
...
/**
 * Règle métier finale : le Badge est réservé aux abonnements récurrents.
 * Aucun Badge pour un paiement comptant ni pour un 4× PayPlug + RIB.
 */
function orderNeedsAutoBadge(order = {}, product = {}) {
  if (isPayplug4xPrelevementOrder(order)) return false;
  if (product.paiement_comptant === true || order.paiement_comptant === true) return false;
  if (isComptantStyleProduct(product) || isComptantStyleProduct(order.product_snapshot || order)) {
    return false;
  }
  if (!productNeedsAutoBadge(product)) return false;

  const payment = order.payment || {};
  const billingPlan = String(order.billing_plan || payment.billing_plan || '').toLowerCase();
  const hasIban = Boolean(
    payment.iban ||
      order.iban ||
      order.customer?.iban ||
      order.customer_short?.iban ||
      order.customer_full?.iban
  );
  return hasIban || billingPlan === 'rib' || billingPlan === 'paypal';
}

function requiresIbanForPlan(product, billingPlan, paymentPlan) {
  if (isPayplug4xPrelevement(paymentPlan, billingPlan)) return true;
  if (String(paymentPlan || '').toLowerCase() === 'scalapay') return false;
  if (isComptantStyleProduct(product)) return false;
...
function paymentModeLabel(product, billingPlan, paymentPlan) {
  ...
  if (installment === '4x' && isPayplug4xPrelevement(installment, billingPlan)) {
    return '4× sans frais — 25 % par CB, puis prélèvement';
  }
  if (installment === 'scalapay') {
    return 'Paiement CB en plusieurs fois (3×/4×)';
  }
  if (installment === '4x') {
...
function paymentTodayVsNextLabel(product, billingPlan, paymentPlan) {
  ...
  if (installment === 'scalapay') {
    return { today: 'Réglé en CB plusieurs fois', next: null };
  }
  if (installment === '4x') {
...
/**
 * Si le RIB Deciplus échoue, ne passer en comptant QUE les offres déjà soldées
 * (259 € / 12 mois, etc.). Un abo 29 € / 4 semaines passé en comptant fait
 * encaisser toute la valeur du contrat (ex. 359,88 €) et ne pose aucune
 * échéance mensuelle — il ne reste que le badge 34,99 €.
 */
function shouldFallbackToComptantOnIbanError(order = {}, productConfig = {}) {
  if (productConfig.paiement_comptant === true) return true;
  const { isOffre29Product, isMonthlyFlexProduct, isAnnualPromoProduct } = require('./sale-contract-match');
  if (isOffre29Product(productConfig) || isOffre29Product(order)) return false;
  if (isMonthlyFlexProduct(productConfig) || isMonthlyFlexProduct(order)) return false;
  // 259 € / 12 mois et 4× Payplug : sans RIB valide le virement Deciplus ne pose pas le contrat.
  if (isAnnualPromoProduct(productConfig) || isAnnualPromoProduct(order)) return true;
  if (isPayplug4xPrelevementOrder(order)) return true;
  if (isComptantStyleProduct(order.product_snapshot || order)) return true;
  return false;
}

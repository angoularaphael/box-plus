...
const { isTrialOrder, isCarteMerchOrder, buildProductConfig } = require('./catalog-sale');
const { requiresIbanForPlan, normalizeBillingPlan, isPayplug4xPrelevement, isPayplug4xPrelevementOrder } = require('./billing-plan');
...
function normalizeAction(input) {
  ...
  if (['inscription_nudge', 'funnel_nudge', 'nudge_inscription'].includes(raw)) {
    return 'inscription_nudge';
  }
  if (['member_photo', 'photo', 'upload_photo'].includes(raw)) {
    return 'member_photo';
  }
  if (['check_sale', 'check_trial_sale', 'seance_offerte_check'].includes(raw)) {
    return 'check_sale';
  }
...
function getJobId(order) {
  ...
  if (order.action === 'inscription_nudge') return `${order.order_id}#inscription_nudge`;
  if (order.action === 'member_photo') return `${order.order_id}#photo`;
  if (order.action === 'check_sale') return `${order.order_id}#check_sale`;
...
function normalizeOrder(input) {
  ...
  const offer = String(
    ...
    .toLowerCase()
    .trim();
  let gym = String(input.gym || input.salle || '').toLowerCase().replace(/\s+/g, '-');
  if (action === 'balma_switch' && !gym) gym = 'minimes';
  const { remapBalmaGymSlug } = require('./gym-slugs');
  gym = remapBalmaGymSlug(gym);
  ...
  const order = {
    ...
    paiement_comptant:
      ...
      payment.payment_plan === 'scalapay' ||
      input.payment_plan === 'scalapay' ||
      (payment.payment_plan === '4x' && !isPayplug4xPrelevementOrder({ payment, billing_plan: input.billing_plan })) ||
      (input.payment_plan === '4x' && !isPayplug4xPrelevement(input.payment_plan, input.billing_plan)) ||
      undefined,
    deciplus_member_id: input.deciplus_member_id ? String(input.deciplus_member_id) : null,
    skip_cancel: Boolean(input.skip_cancel),
    skip_migrate: Boolean(input.skip_migrate),
    skip_restore: Boolean(input.skip_restore),
    offer: input.offer || null,
    snapshots: Array.isArray(input.snapshots) ? input.snapshots : undefined,
    ...
    sales_bot: input.sales_bot || null,
    failover_count: Math.max(0, Number(input.failover_count || 0)),
    failover_from: input.failover_from || null,
    failover_at: input.failover_at || null,
    failover_reason: input.failover_reason || null,
    force_requeue: Boolean(input.force_requeue),
    force_sale_retry: Boolean(input.force_sale_retry),
    force_new_member: Boolean(input.force_new_member),
    ...
    customer: {
      ...
      guardian: customer.guardian || input.guardian || null,
    },
    payment: {
      amount: Number(
        payment.amount ??
    ...
    is_friend_referral: Boolean(input.is_friend_referral),
    aventure: Boolean(input.aventure),
    ready_for_dispatch: Boolean(input.ready_for_dispatch),
    signature: input.signature?.signed_at ? { signed_at: input.signature.signed_at } : null,
...
function validateOrder(order) {
  ...
  if (order.action === 'inscription_nudge') {
    const errors = [];
    if (!order.order_id) errors.push('order_id manquant');
    if (!order.customer?.email && !order.email) errors.push('email manquant');
    return errors;
  }
  if (order.action === 'member_photo') {
    ...
    if (!order.customer?.first_name) errors.push('prénom manquant');
    if (!order.customer?.last_name) errors.push('nom manquant');
    if (!order.gym) errors.push('salle (gym) manquante');
    if (!order.photo_base64 && !order.photo_url && !order.photo_path) errors.push('photo manquante');
    return errors;
  }
  if (order.action === 'check_sale') {
    const errors = [];
    if (!order.order_id) errors.push('order_id manquant');
    if (!order.deciplus_member_id && !order.customer?.last_name) errors.push('membre introuvable');
    return errors;
  }
  ...
  if (order.action === 'encaisser') {
    const errors = [];
    if (!order.order_id) errors.push('order_id manquant');
    if (!order.deciplus_member_id) errors.push('deciplus_member_id manquant pour encaisser');
    return errors;
  }
  const errors = [];
  if (!order.order_id) errors.push('order_id manquant');
  if (!order.customer.first_name) errors.push('prénom manquant');
  if (!order.customer.last_name) errors.push('nom manquant');
  if (!order.customer.email && !order.customer.phone && !order.deciplus_member_id) {
  ...
  if (order.payment.iban && !isValidFrenchIban(order.payment.iban)) {
    if (String(order.payment.status || '').toLowerCase() === 'paid') {
      // 1er mois déjà encaissé : ne pas bloquer la fiche Deciplus (RIB à reprendre).
      order.payment.iban = null;
    } else {
  ...
  const carteMerch = isCarteMerchOrder(order);
  const comptantLike =
    !payplug4xPrelev &&
    (carteMerch ||
    ...
    order.sale_type === 'carte' ||
    /comptant/i.test(order.product_name || '') ||
    /OFFRE\s*PROMO\s*12\s*MOIS/i.test(order.product_name || '') ||
    /BABY\s*BOXE/i.test(order.product_name || '') ||
    /BOXE\s*EDUCATIVE/i.test(order.product_name || ''));

  const billingPlan = normalizeBillingPlan(
    order.payment?.billing_plan || order.billing_plan,
    {
      requires_iban: carteMerch || order.requires_iban === false ? false : order.requires_iban,
  ...
  const needsIban =
    ...
    requiresIbanForPlan(
      {
        requires_iban: carteMerch || order.requires_iban === false ? false : order.requires_iban !== false,
        name: order.product_name || '',
        id: order.product_id,
      },
      billingPlan,
  ...
  if (needsIban && !order.payment.iban && order.payment.status !== 'paid') {

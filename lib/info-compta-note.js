...
function isFourXOrder(order = {}, productConfig = {}) {
  ...
  const hint = [
    ...
    .filter(Boolean)
    .join(' ');
  // Scalapay = encaissement comptant (montant total) — pas une note d’échéancier 4× club
  return /4\s*[x×]\s*sans\s*frais/i.test(hint) && plan !== 'once' && plan !== 'scalapay';
}
...
function buildPaymentChannelInfoComptaNote(order = {}) {
  ...
  const { isPayplug4xPrelevementOrder } = require('./billing-plan');
  if (isPayplug4xPrelevementOrder(order) || method === 'payplug' || method === 'cawl') {
    return method === 'cawl' ? '4× sans frais CB' : '4× sans frais CB';
  }
